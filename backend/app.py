...
@app.route('/smart_split', methods=['POST'])
def smart_split():
    text = request.json.get('text', '')
    # Heurística: si el texto es corto o tiene muchos saltos dobles, usa algoritmo local
    if len(text) < 1200 or text.count('\n\n') > 2:
        # Algoritmo local
        parts = [p.strip() for p in re.split(r'\n\n+', text) if p.strip()]
        return jsonify({'parts': parts})
    # Si es largo o complejo, usa Gemini
    parts = split_paragraphs_with_gemini(text)
    if parts and isinstance(parts, list) and all(isinstance(p, str) for p in parts):
        return jsonify({'parts': parts})
